...
fn setup(sketch: &mut Sketch<MyState>) {
    ...
    sketch.state.transition = Some(Transition::initialize(
        EasingType::QuadOut,
        3.0,
        TransitionTarget::Points {points: points.clone()},
        TransitionTarget::Points {points},
    ));
...
fn next_iteration(transition: &mut Transition) {
    ...
    let next_start = TransitionTarget::Points {points: next_start};
    let next_end = TransitionTarget::Points {points: next_end};
